...
def get_llm_chain(llm_model, prompt_template):
    pipeline = (
        ...
        | prompt_template
        | llm_model.bind(stop=["Human:", "Sinh viên:", "Học sinh:"])
    )
    return pipeline
...
class ChatChain:
    ...
    def run(self, question, memory):
        return self.llm_chain.invoke(
            input={"human_input": question, "history": memory.load_memory_variables({})}
        )
...
def get_retriever_chain(llm_model, retriever):
    return RetrievalQA.from_chain_type(
        llm=llm_model, chain_type="stuff", retriever=retriever, verbose=True
    )
...
class RAG_ChatChain:
    def __init__(
        ...
        self.llm_chain = get_retriever_chain_pipeline(
            llm_model=llm_model, prompt_template=prompt_template, retriever=retriever
        )
