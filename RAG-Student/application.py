...
def Chatbot():
    ...
    def history_choice_show(emp):
        typewriter_effect(
            emp, "Session hiện tại: " + st.session_state.history_choice, delay=0.00001
        )
        with emp:
    ...
    with st.sidebar:
        ...
        if rag_button:
            ...
            number_of_documents = st.sidebar.number_input(
                label="Số documents cung cấp", min_value=1, max_value=10
            )
        ...
        history_choosing = st.sidebar.selectbox(
            "Select a chat session",
            list_chat_sessions,
            key="chat_session",
            on_change=history_choosing_change,
        )
...
def Databases():
    ...
    with st.sidebar:
        ...
        data_type_option = st.sidebar.selectbox(
            "Select a data type",
            ["Document (PDF, Word)", "History Chat"],
            key="data_type",
        )
        if data_type_option == "Document (PDF, Word)":
            document_add = st.file_uploader(
                label="Import File to database", type=["pdf", "docx"]
            )
            if document_add is not None:
                ...
                if upload_button:
                    path = (
                        os.path.join(db_config["pdf_path"], document_add.name)
                        if document_add.name.endswith(".pdf")
                        else os.path.join(db_config["word_path"], document_add.name)
                    )
                    with open(path, "wb") as f:
                        f.write(document_add.getbuffer())
                    st.success(
                        f"File {document_add.name} đã được tải lên và lưu trữ thành công!"
                    )

    if data_type_option == "Document (PDF, Word)":
        st.session_state.document_details = st.selectbox(
            "Select a document", get_list_documents(), key="list_documents"
        )
        if st.session_state.document_details is not None:
            ...
            with but_col2:
                st.download_button(
                    ...
                    mime=(
                        "application/pdf"
                        if st.session_state.document_details.endswith(".pdf")
            ...
            if delete_button:
                if delete_document(st.session_state.document_details):
                    st.success(
                        "Đã xóa thành công document trong hệ thống! Nếu là ấn nhầm bạn vẫn có thể tải xuống ngay lúc này!"
                    )
                else:
